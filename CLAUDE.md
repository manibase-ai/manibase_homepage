...
- **Einmalige Provisionierung:** `infra/provision-manibase.sh` (legt Docroot + nginx-vhost mit Security-Headern/Calendly-CSP an, holt Let's-Encrypt-Cert). **Braucht root** (deploy hat KEIN passwordless sudo) → `sudo bash ~/provision-manibase.sh`. Voraussetzung: DNS-A-Record `manibase.aicoreinfra.de → 72.62.42.27` muss vorher gesetzt sein (certbot HTTP-01).
