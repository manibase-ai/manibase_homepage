...
- **P1-Fixes umgesetzt (Juli 2026):** #2 Newsletter → echter POST an `/api/newsletter` (PHP-Proxy → Odoo, **Single-Opt-In**; DOI später via Odoo-App *Marketing Automation* nachrüstbar). #3 Wizard-Antworten werden per **Zeeg**-Prefill übergeben (kein Datenverlust). #4 Datenschutz auf Realität abgeglichen (Hostinger/Zeeg/Odoo).
...
- **Offen vor endgültigem Abschluss:** `username`+`api_key` in `/etc/manibase/odoo.php` eintragen (User); AVV Hostinger/Odoo abschließen, Zeeg-AVV liegt bei; Staging abbauen (`teardown-staging.sh` als root auf altem Server) + alten DNS-Record löschen.
